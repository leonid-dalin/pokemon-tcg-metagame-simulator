...
def compute_convergence_metrics(
    ...
    with tracer.start_as_current_span("compute_convergence_metrics"):
        ...
        logger.info(
            "convergence_metrics_computed",
            generation=convergence_gen if convergence_gen is not None else "N/A",
            avg_post_conv_change=float(avg_post_conv)
        )
        return metrics
...
def generate_final_state_tier_list(
    ...
    with tracer.start_as_current_span("generate_final_state_tier_list"):
        n = len(deck_names)
        ...
        logger.info("tier_list_generated", active_decks=sum(len(t) for t in tiers.values()))
        for tier in TIER_ORDER:
            if tiers[tier]:
                ...
                logger.debug(
                    ...
                    tier=tier,
                    deck=top_deck,
                    win_rate=float(tiers[tier][0]['win_rate'])
                )
...
def compute_matchup_cycles(
    ...
    with tracer.start_as_current_span("compute_matchup_cycles"):
        n = len(deck_names)
        ...
        cycles = []
        if cycle_length != 3:
            logger.warning("cycle_length_unsupported", requested_length=cycle_length,
                           detail="Only 3-cycles implemented currently.")
            return cycles
...
def compute_deck_similarity(
    ...
    with tracer.start_as_current_span("compute_deck_similarity"):
        n = len(deck_names)
        ...
        for i, (sim_val, r, c) in enumerate(pairs[:5]):
            ...
            logger.debug(
                "deck_similarity_pair",
                rank=i + 1,
                deck_a=deck_names[r],
                deck_b=deck_names[c],
                pearson_correlation=float(sim_val)
            )
